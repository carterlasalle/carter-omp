...
from carter_omp import github_events, issue_index
from carter_omp.autoclose import AutocloseScheduler
from carter_omp.config import Settings, get_settings
from carter_omp.dashboard import render_index, static_dir, tail_jsonl
from carter_omp.db import (
...
from carter_omp.github_client import GitHubError, IssueSummary
from carter_omp.github_events import RouteDecision
from carter_omp.issue_index import IssueIndexSync
...
def _record_decision_and_run(
    ...
    """Persist the authorization decision and, when queued, the run record.

    Every event — including rejected ones — is recorded without invoking
    the model. The run row answers who/what/why/capabilities for the audit log.
    """
    from carter_omp.capabilities import POLICY_VERSION

    action = str(payload.get("action") or "")
    authz = decision.authz
    actor = authz.actor if authz is not None else None
    repo = payload.get("repository")
    repo_id = repo.get("id") if isinstance(repo, Mapping) else None
    db.record_authorization_decision(
        ...
        action=action,
        trigger_kind=authz.trigger_kind if authz is not None else None,
        authorized=bool(authz.authorized) if authz is not None else bool(decision.should_queue),
        reason=decision.reason,
        policy_version=POLICY_VERSION,
    )
    trigger = decision.trigger
    if trigger is not None and decision.should_queue:
        db.record_run(
            ...
            pr_number=trigger.pull_request_number,
            actor_id=trigger.actor_id,
            actor_login=trigger.actor_login,
            trigger_kind=trigger.trigger_kind,
            trigger_object_id=trigger.trigger_object_id,
            trigger_value=trigger.trigger_value,
            policy_version=trigger.policy_version,
            capabilities_json=json.dumps(sorted(c.value for c in trigger.capabilities)),
            authorized_at=trigger.authorized_at.isoformat(),
        )
...
def create_app(settings: Settings | None = None, *, pool_factory: _PoolFactory = _build_worker_pool) -> FastAPI:
    ...
    app = FastAPI(title="carter_omp", version="0.1.0", lifespan=lifespan)
    ...
    async def replay(
        request: Request,
        x_carter_omp_token: str | None = Header(None, alias="X-CarterOmp-Replay-Token"),
        delivery_id: str = "",
    ) -> JSONResponse:
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        ...
        if x_carter_omp_token != cfg.replay_token.get_secret_value():
            raise HTTPException(401, "invalid replay token")
        db: Database = bag["db"]
        row = db.get_event(delivery_id)
        if row is None:
            raise HTTPException(404, "unknown delivery")
        # Replay reuses the stored trigger context and never re-authorizes.
        # A skipped event (never authorized) stays unauthorized.
        if row.state == "skipped" or "_carter_omp_trigger" not in row.payload:
            raise HTTPException(409, f"delivery {delivery_id} was never authorized; replay refused")
        if not db.requeue_event(delivery_id, from_states=("done", "failed")):
            raise HTTPException(409, f"delivery {delivery_id} is {row.state}; only done/failed events can be replayed")
        log.info("replay", extra={"delivery": delivery_id})
        bag["pool"].wake()
        return JSONResponse({"delivery": delivery_id, "state": "queued"})

    def _require_trigger_token(cfg: Settings, token: str | None) -> None:
        if cfg.replay_token is None:
            raise HTTPException(404, "trigger disabled (set CARTER_OMP_REPLAY_TOKEN to enable)")
    ...
    async def api_github_issues(
        ...
        """Browse issues across `CARTER_OMP_REPO_ALLOWLIST` for the trigger picker.
        ...
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        _require_trigger_token(cfg, x_carter_omp_token)
    ...
    async def api_trigger(
        ...
        if mode == "triage":
            ...
            if not cfg.allows(repo_full):
                raise HTTPException(403, f"{repo_full} not in CARTER_OMP_REPO_ALLOWLIST")
            try:
    ...
    async def api_status(request: Request) -> dict[str, Any]:
        ...
        def _collect() -> dict[str, Any]:
            ...
            def _authz_payload(delivery_id: str) -> dict[str, Any]:
                decision = db.get_authorization_decision(delivery_id)
                run = db.get_run_by_delivery(delivery_id)
                out: dict[str, Any] = {}
                if decision is not None:
                    out["actor_login"] = decision.get("actor_login")
                    out["actor_id"] = decision.get("actor_id")
                    out["authorized"] = bool(decision.get("authorized"))
                    out["auth_reason"] = decision.get("reason")
                    out["trigger_kind"] = decision.get("trigger_kind")
                if run is not None:
                    out["run_id"] = run.get("id")
                    out["capabilities"] = json.loads(str(run.get("capabilities_json") or "[]"))
                return out
            ...
            def _latest_event_payload(key: str) -> dict[str, Any] | None:
                ...
                return {
                    ...
                    **_authz_payload(latest.delivery_id),
                }
    ...
    async def api_logs(request: Request, limit: int = 400) -> dict[str, Any]:
        ...
        entries = tail_jsonl(cfg.log_dir / "carter_omp.log.jsonl", limit=capped)
