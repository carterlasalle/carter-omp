...
CREATE INDEX IF NOT EXISTS runs_delivery ON runs(delivery_id);
...
class Database:
    ...
    def has_authorized_impl_event(self, issue_key: str) -> bool:
        ...
        for row in rows:
            payload = json.loads(row["payload_json"])
            directive = payload.get("_carter_omp_directive")
    ...
    def processed_issue_keys(self, keys: Iterable[str]) -> set[str]:
        ...
        Membership in `issues` means carter_omp has at minimum upserted state for the
        ...
        with self._lock:
            self._conn.execute(
                ...
                INSERT OR REPLACE INTO runs
                  (id, delivery_id, repo_id, repo, issue_number, pr_number,
                   actor_id, actor_login, trigger_kind, trigger_object_id,
                   trigger_value, policy_version, capabilities_json,
                   authorized_at, state)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
        ...
        with self._lock:
            self._conn.execute(
                ...
                INSERT OR REPLACE INTO authorization_decisions
                  (delivery_id, repo_id, actor_id, actor_login, event_type,
                   action, trigger_kind, authorized, reason, policy_version,
                   created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
    ...
    def get_authorization_decision(self, delivery_id: str) -> dict[str, object] | None:
        with self._lock:
            row = self._conn.execute(
                "SELECT * FROM authorization_decisions WHERE delivery_id = ?",
                (delivery_id,),
            ).fetchone()
        return dict(row) if row is not None else None

    def get_run_by_delivery(self, delivery_id: str) -> dict[str, object] | None:
        with self._lock:
            row = self._conn.execute(
                "SELECT * FROM runs WHERE delivery_id = ?",
                (delivery_id,),
            ).fetchone()
        return dict(row) if row is not None else None
