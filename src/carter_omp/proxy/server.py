"""github-proxy FastAPI app: HMAC-gated GitHub REST + git proxy.

CarterOmp calls every endpoint with HMAC headers (see `carter_omp.proxy_hmac`).
Authenticated requests dispatch to a single `GitHubClient` instance holding
the PAT, or to `carter_omp.git_ops` for git transport. The PAT never leaves
this process.

Endpoint payloads are deliberately typed (no generic GitHub passthrough):
each one names exactly one operation carter_omp performs.
"""
...
from carter_omp.sandbox import workspace_key as compute_workspace_key
...
def _require_fetch_ref(value: Any) -> str:
    ...
    `carter-omp/x/y`, `alice/fix-parser`) or `refs/heads/<name>`. Reject anything
...
def _resolve_token(cfg: Settings) -> str:
    # PAT fallback. In App mode the per-request scoped client carries the
    # installation token; these call sites only run when no App provider
    # is configured (or for the startup identity probe).
    if cfg.github_token is None:
        # Will already have been caught at startup, but stay defensive.
        raise HTTPException(500, "github-proxy: GITHUB_TOKEN not configured")
...
# trace:v1 id=impl.proxy-app-scoped work=WORK-CO-Q8Z1HJJJ satisfies=REQ-CO-9N23MPRP
def _scoped_client(request: Request, repo: str) -> GitHubClient:
    """Return a GitHub client scoped to `repo` for this request.

    GitHub App mode: mints a short-lived installation token for the repo
    (cached until before expiry). PAT mode: the shared client. Either way
    the caller also needs a valid run token (checked by the endpoint).
    """
...
def _resolve_hmac_key(cfg: Settings) -> bytes:
    if cfg.github_proxy_hmac_key is None:
        raise HTTPException(500, "github-proxy: CARTER_OMP_GH_PROXY_HMAC_KEY not configured")
    return cfg.github_proxy_hmac_key.get_secret_value().encode("utf-8")
...
def _require_run_token(request: Request, cfg: Settings) -> RunToken:
    """Verify BOTH the orchestrator HMAC (already checked) and the run token."""
    token = verify_run_token(
        key=_resolve_hmac_key(cfg),
        token=request.headers.get(HEADER_RUN_TOKEN),
    )
    if token is None:
...
def _require_run_cap(token: RunToken, capability: str) -> None:
    if capability not in token.capabilities:
        raise HTTPException(403, f"run lacks capability {capability}")


def _require_run_repo(token: RunToken, repo: str) -> None:
    if token.repo.lower() != repo.lower():
...
def _require_push_namespace(branch: str) -> None:
    """Normal pushes land only on `carter-omp/*`; releases use the elevated endpoint."""
    if not branch.startswith("carter-omp/"):
        raise HTTPException(403, "branch outside carter-omp/* namespace")
...
def _read_remote_urls(repo_dir: Path, slot_uid: int | None = None, *, push: bool = False) -> list[str]:
    ...
    if proc.returncode != 0:
        ...
        log.warning("github-proxy: failed to read origin url", extra={"repo_dir": str(repo_dir)})
...
def _read_single_remote_url(repo_dir: Path, expected_repo: str, *, push: bool, slot_uid: int | None = None) -> str:
    ...
    if len(urls) != 1:
        ...
        log.warning(
            "github-proxy: refusing git op — origin has ambiguous remote urls",
...
def _clone_remote_auth(clone_url: str, expected_repo: str, token: str) -> _RemoteAuth:
    try:
        ...
        log.warning(
            "github-proxy: refusing clone — clone_url is not permitted",
...
def _origin_remote_auth(
    ...
    *,
    ...
    try:
        ...
        log.warning(
            "github-proxy: refusing git op — origin url is not permitted",
...
def create_proxy_app(settings: Settings) -> FastAPI:
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        provider: AppTokenProvider | None = None
        key_file = settings.github_app_private_key_file
        if settings.github_app_id is not None and key_file is not None:
            try:
                pem = Path(key_file).read_text(encoding="utf-8")
                if not pem.strip():
                    raise ValueError("empty private key file")
                provider = AppTokenProvider(app_id=settings.github_app_id, private_key_pem=pem)
                app.state.app_token_provider = provider
                object.__setattr__(settings, "_app_token_provider", provider)
    ...
    app = FastAPI(title="carter-omp-github-proxy", version="0.1.0", lifespan=lifespan)
    ...
    async def _read_body_capped(request: Request) -> bytes:
        ...
        """
        max_bytes = settings.github_proxy_max_body_bytes
    ...
    async def get_repo(request: Request, repo: str) -> JSONResponse:
        await _authenticate(request)
        _enforce_repo_scope(request.app.state.settings, repo)
    ...
    async def open_pull_request(request: Request) -> JSONResponse:
        ...
        _enforce_repo_scope(cfg, repo)
        _require_run_cap(token, "open_pr")
        _require_run_repo(token, repo)
        _require_run_branch(token, head)
    ...
    async def request_reviewers(request: Request) -> JSONResponse:
        ...
        _enforce_repo_scope(cfg, repo)
        _require_run_cap(token, "request_review")
        _require_run_repo(token, repo)
        _require_run_thread(token, pr_number)
    ...
    async def submit_pr_review(request: Request) -> JSONResponse:
        ...
        pr_number = _require_int(data.get("pr_number"), "pr_number")
        _enforce_repo_scope(cfg, repo)
        _require_run_cap(token, "review_pr")
        _require_run_repo(token, repo)
        _require_run_thread(token, pr_number)
        body = _require_str(data.get("body"), "body")
    ...
    # ---- git transport ----
    #
    # The underlying `carter_omp.git_ops` primitives are blocking `subprocess.run`
    ...
    async def _run_git_op(fn, *args, **kwargs):  # type: ignore[no-untyped-def]
        try:
            return await asyncio.wait_for(
                ...
                timeout=settings.github_proxy_git_timeout_seconds,
            )
        except TimeoutError as exc:
            log.warning(
                "github-proxy: git op exceeded timeout",
                extra={"op": fn.__name__, "timeout": settings.github_proxy_git_timeout_seconds},
            )
    ...
    async def git_clone_endpoint(request: Request) -> JSONResponse:
        data = await _json_body(request)
        _enforce_repo_scope(request.app.state.settings, _require_str(data.get("repo"), "repo"))
        repo = _require_str(data.get("repo"), "repo")
        clone_url = _require_str(data.get("clone_url"), "clone_url")
        default_branch = _require_str(data.get("default_branch"), "default_branch")
        remote = _clone_remote_auth(clone_url, repo, _resolve_git_token(settings, repo))
    ...
    async def git_fetch_endpoint(request: Request) -> JSONResponse:
        ...
        repo = _require_str(data.get("repo"), "repo")
        target = _pool_dir(settings, repo)
        remote = await asyncio.to_thread(_origin_remote_auth, target, repo, _resolve_git_token(settings, repo))
        try:
            await _run_git_op(
                git_fetch_prune,
                target,
    ...
    async def git_push_endpoint(request: Request) -> JSONResponse:
        ...
        _enforce_repo_scope(cfg, repo)
        _require_run_cap(token, "push_branch")
        _require_run_repo(token, repo)
        _require_run_branch(token, branch)
        _require_push_namespace(branch)
    ...
    async def git_push_release_endpoint(request: Request) -> JSONResponse:
        ...
        repo = _require_str(data.get("repo"), "repo")
        _enforce_repo_scope(cfg, repo)
        _require_run_cap(token, "update_default_branch")
        _require_run_cap(token, "move_release_tag")
        _require_run_repo(token, repo)
        workspace_key = _require_str(data.get("workspace_key"), "workspace_key")
        branch = _require_branch(data.get("branch"))
