...
from carter_omp import persona
...
class ToolBindings:
    ...
    # Fixed capabilities derived by trusted routing code from the accepted
    # trigger. The model cannot request more; every side-effect tool calls
    # `require()` first. Empty set = no GitHub/publish side effects allowed.
    capabilities: frozenset[Capability] = frozenset()
    # Immutable trigger record for this run, if any. Kept for audit context;
    # authorization itself lives in `capabilities`, not in this object.
    ...
    def default_comment_number(self) -> int:
        ...
        # trace:v1 id=impl.capability-require work=WORK-CO-Q8Z1HJJJ satisfies=REQ-CO-9N23MPRP

    def require(self, capability: Capability, tool_name: str, args: Mapping[str, Any]) -> None:
        """Raise unless this run holds `capability`. The ultimate security boundary."""
        if capability not in self.capabilities:
            msg = f"refusing: run lacks capability {capability.value}."
            _audit(self, tool_name, args, error=msg)
            _raise_command(msg)
...
def _run_pre_publish_bun_fix(
    ...
    *,
    ...
    When the run holds the SKIP_CHECKS capability (granted only by an explicit
    operator directive parsed before the model runs), the formatter is NOT invoked and any post-fix commit is skipped, so a
    ...
    if pre_status.stdout.strip():
        ...
        _raise_command(msg)
    if Capability.SKIP_CHECKS in bindings.capabilities:
        _audit(
            bindings,
            tool_name,
            args,
            result={"skipped": "bun_run_fix", "reason": "capability:skip_checks"},
        )
        return
    try:
    ...
    if ahead.returncode != 0 or not ahead.stdout.strip():
        msg = (
            ...
            "(`git checkout -- . && git clean -fd`) and report the pre-existing breakage."
        )
        _audit(bindings, tool_name, args, error=msg)
        _raise_command(msg)
...
def _run_pre_publish_bun_check(
    ...
    *,
    ...
    """Run `bun check` before publishing. When the run holds SKIP_CHECKS the check
    ...
    if Capability.SKIP_CHECKS in bindings.capabilities:
        _audit(
            ...
            tool_name,
            args,
            result={"skipped": "bun_check", "reason": "capability:skip_checks"},
        )
        return
...
def _run_pre_publish_bun_test(
    ...
    *,
    ...
    `scripts.test`, bypassed by the SKIP_CHECKS capability for breakage the agent's
    ...
    if Capability.SKIP_CHECKS in bindings.capabilities:
        _audit(
            ...
            tool_name,
            args,
            result={"skipped": "bun_run_test", "reason": "capability:skip_checks"},
        )
        return
...
def _repair_commit_message_escapes(bindings: ToolBindings, args: Mapping[str, Any], *, tool_name: str) -> None:
    ...
    update = _run_repo_command(
        bindings,
        ["git", "update-ref", "-m", "carter_omp: repaired commit message escapes", "HEAD", new_head, old_head],
    )
...
def _guarded_push_branch(bindings: ToolBindings, args: Mapping[str, Any], tool_name: str, branch: str) -> str:
    ...
    try:
        ...
        msg = f"github-proxy rejected push: {exc.status} {exc.message}"
        _audit(bindings, tool_name, args, error=msg)
        _raise_command(msg)
...
def _build_release_retag(bindings: ToolBindings) -> HostTool[Any, Any]:
    ...
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> dict[str, Any]:
        ...
        try:
            ...
            refuse(args, f"github-proxy rejected push: {exc.status} {exc.message}")
...
def _build_push_branch(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        ...
        skip = Capability.SKIP_CHECKS in bindings.capabilities
        ...
        # The SKIP_CHECKS capability (never a model arg) bypasses the
        # formatter/check (e.g. when `main` itself is broken); dirty-tree
        # gate still runs unconditionally.
...
def _build_open_pr(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        ...
        _run_pre_publish_bun_test(bindings, args, tool_name="gh_open_pr", stage="open PR")
...
def _build_request_review(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        bindings.require(Capability.REQUEST_REVIEW, "gh_request_review", args)
...
def _enforce_impl_authorization(
    ...
    *,
    ...
    msg = (
        ...
        "an explicit authorized trigger must start this run before any branch/PR. "
        "Post your analysis with `gh_post_comment` and stop."
    )
    _audit(bindings, tool_name, args, error=msg)
    _raise_command(msg)
...
def _require_review_mode(bindings: ToolBindings, name: str, args: Mapping[str, Any]) -> None:
    bindings.require(Capability.REVIEW_PR, name, args)
    if bindings.review_mode:
        return
...
def _build_submit_pr_review(bindings: ToolBindings) -> HostTool[Any, Any]:
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        ...
        comments: list[Mapping[str, Any]] = [_review_comment_to_payload(comment) for comment in staged]
...
def _build_set_issue_labels(bindings: ToolBindings) -> HostTool[Any, Any]:
    ...
    def execute(args: dict[str, Any], _ctx: HostToolContext[Any]) -> str:
        bindings.require(Capability.LABEL, "set_issue_labels", args)
        if bindings.inbound_is_pr:
...
# trace:v1 id=impl.capability-gated-tools work=WORK-CO-Q8Z1HJJJ satisfies=REQ-CO-9N23MPRP
def build(bindings: ToolBindings) -> tuple[HostTool[Any, Any], ...]:
    """Return the capability-gated host tools for one task's run.

    Security wins over prompt-cache warmth: a tool the run is not authorized
    for is never exposed, not merely described as off-limits. Every exposed
    side-effect tool still enforces its capability again at execution time.
    """
    caps = bindings.capabilities
    tools: list[HostTool[Any, Any]] = [
        _build_fetch_thread(bindings),
        _build_search_issues(bindings),
        _build_search_commits(bindings),
        _build_abort_task(bindings),
    ]
    if Capability.COMMENT in caps:
        tools.append(_build_post_comment(bindings))
    if Capability.LABEL in caps:
        tools += [_build_classify_issue(bindings), _build_set_issue_labels(bindings)]
    if Capability.REVIEW_PR in caps:
        tools += [
    ...
    if Capability.PUSH_BRANCH in caps:
        tools.append(_build_push_branch(bindings))
    if Capability.OPEN_PR in caps:
    ...
    if Capability.UPDATE_DEFAULT_BRANCH in caps and Capability.MOVE_RELEASE_TAG in caps:
        tools.append(_build_release_retag(bindings))
    if Capability.EDIT_WORKTREE in caps:
        tools += [_build_repro_record(bindings), _build_mark_unable(bindings)]
    return tuple(tools)
