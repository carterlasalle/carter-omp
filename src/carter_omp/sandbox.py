...
`GitTransport` so a deploy can keep the PAT entirely in a separate `github-proxy`
container. The default `LocalGitTransport` runs git in-process with ephemeral
PAT injection via `--config-env` (see `carter_omp.git_ops`); the `ProxyGitTransport`
in `carter_omp.proxy_client` forwards the same set of operations over HMAC RPC.
...
from carter_omp.natives_cache import CacheHit, NativesCache
...
def rename_workspace_branch(
    ...
    *,
    ...
    """Rename the workspace's local branch to ``carter-omp/<hex>/<new_slug>``.
    ...
    workspace whose branch isn't on the ``carter-omp/<hex>/<slug>`` shape.
    ...
    if len(parts) != 3 or parts[0] != "carter-omp" or not parts[1]:
        raise ValueError(f"refusing to rename non-carter-omp branch {workspace.branch!r}")
    new_branch = f"carter-omp/{parts[1]}/{new_slug}"
