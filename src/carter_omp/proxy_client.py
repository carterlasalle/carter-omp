"""Client half of the carter-omp ↔ github-proxy channel.

`GitHubProxyClient` implements `GitHubBackend` by HMAC-signing each request
and forwarding to github-proxy. `ProxyGitTransport` implements `GitTransport` by
routing clone/fetch/push through the proxy too — carter-omp never holds the PAT.
...
from carter_omp.git_ops import GitCommandError, HeadDriftError, PushResult
...
def _decode_error(resp: httpx.Response) -> Exception:
    """Map a non-2xx response from github-proxy back to a domain exception.
            ...
            try:
                async with self._async_client() as client:
                    ...
                    if self._run_token is not None:
                        req.headers[HEADER_RUN_TOKEN] = self._run_token
...
class ProxyGitTransport:
    ...
    __slots__ = ("_base_url", "_key", "_transport", "_timeout", "_run_token")
    ...
    def _post(self, path: str, body: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        for attempt, delay in enumerate((*self._TRANSIENT_RETRY_DELAYS, None)):
            try:
                ...
                if self._run_token is not None:
                    headers[HEADER_RUN_TOKEN] = self._run_token
