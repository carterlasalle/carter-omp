...
from carter_omp.db import Database, IssueRow, IssueState, issue_key
from carter_omp.git_ops import rev_parse_head
...
from carter_omp.sandbox import GitTransport, SandboxManager
from carter_omp.worker import DirectiveInfo, ReleaseTaskContext, TaskInputs, ThreadMessage, run_task

log = logging.getLogger(__name__)


async def _run_workspace_op[T](func: Callable[..., T], /, **kwargs: object) -> T:
...
def _directive_from_payload(payload: Mapping[str, Any]) -> DirectiveInfo | None:
    ...
    raw = payload.get("_carter_omp_directive")
...
def _trigger_from_payload(payload: Mapping[str, Any]) -> object | None:
    """Rehydrate the stored trigger record, if any. Never re-authorizes."""
    from carter_omp.github_events import TriggerContext

    raw = payload.get("_carter_omp_trigger")
    if not isinstance(raw, Mapping):
        return None
    try:
        return TriggerContext.from_record(raw)
    except Exception:
        return None
...
async def handle_pr_conversation(
    *,
    ...
    # _carter_omp_directive; _directive_from_payload rejects it but the key
    # being present tells us a mention happened. Reply cheaply without omp.
    if directive is None and payload.get("_carter_omp_directive") is not None:
        comment = _comment_from_payload(payload)
        log.info(
