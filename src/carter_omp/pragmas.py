...
```
@carter_omp-bot /model gpt /thinking low
...
- `/model <alias>` — pick the first id in `CARTER_OMP_MODEL` whose model id
  contains `<alias>` (case-insensitive). Falls back to the normal random
  pool selection if no member matches.
- `/thinking <level>` — override `CARTER_OMP_THINKING` for this run. Accepts
