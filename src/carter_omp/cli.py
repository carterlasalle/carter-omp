...
def init_models(
    ...
    for selector in pool:
        ...
        sel_provider = selector.split("/")[0] if "/" in selector else ""
        if sel_provider and sel_provider != provider:
            click.echo(
                f"provider/model mismatch: --provider {provider} but selector {selector}; "
    ...
    if gateway:
        lines = [
            ...
            "# Model selection lives in .env (CARTER_OMP_MODEL/_THINKING).",
            "providers:",
            f"  {provider}:",
            f"    baseUrl: {gateway}",
            "    transport: pi-native",
        ...
        lines = [
            ...
            "# lives in .env (CARTER_OMP_MODEL/_THINKING).",
        ]
    lines.append("")
