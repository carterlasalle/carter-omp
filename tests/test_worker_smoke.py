...
Runs only when CARTER_OMP_INTEGRATION=1 and `omp` is available on PATH (or via
CARTER_OMP_OMP_COMMAND). Spins up:
...
INTEGRATION = os.environ.get("CARTER_OMP_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not INTEGRATION,
    reason="CARTER_OMP_INTEGRATION=1 required to run the omp-backed smoke test",
)
...
def test_triage_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from carter_omp.tasks import triage_issue

    bare = _seed_failing_repo(tmp_path)

    monkeypatch.setenv("CARTER_OMP_GH_PROXY_URL", "http://github-proxy.invalid:8081")
    monkeypatch.setenv("CARTER_OMP_GH_PROXY_HMAC_KEY", "test-hmac-key-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    monkeypatch.setenv("GITHUB_TOKEN", "")
    monkeypatch.setenv("GITHUB_WEBHOOK_SECRET", "secret")
    monkeypatch.setenv("CARTER_OMP_BOT_LOGIN", "carter_omp-bot")
    monkeypatch.setenv("CARTER_OMP_REPO_ALLOWLIST", "octo/widget")
    monkeypatch.setenv("CARTER_OMP_WORKSPACE_ROOT", str(tmp_path / "workspaces"))
    monkeypatch.setenv("CARTER_OMP_SQLITE_PATH", str(tmp_path / "carter-omp.sqlite"))
    monkeypatch.setenv("CARTER_OMP_LOG_DIR", str(tmp_path / "logs"))
    monkeypatch.setenv("CARTER_OMP_TASK_TIMEOUT_SECONDS", "300")
