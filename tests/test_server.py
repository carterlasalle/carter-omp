...
from carter_omp.config import Settings, reset_settings_cache
...
def _seed_db(settings: Settings) -> None:
    ...
    db.upsert_issue(
        ...
        number=3,
        state="opened",
        branch="carter-omp/abc12345/fix",
        pr_number=42,
    )
...
def test_index_substitutes_replay_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CARTER_OMP_REPLAY_TOKEN", "secret-token-7")
...
def test_api_status_reports_runtime_counts_and_inflight(settings: Settings) -> None:
    ...
    assert runtime["bot_login"] == "carter_omp-bot"
    ...
    assert issues[fix_key]["branch"] == "carter-omp/abc12345/fix"
...
def test_api_logs_tails_jsonl_file(settings: Settings) -> None:
    log_path = settings.log_dir / "carter_omp.log.jsonl"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    payloads = [
        {"ts": "2026-05-14T21:28:28Z", "level": "INFO", "logger": "carter_omp.queue", "msg": "dispatch loop online"},
        {
            "ts": "2026-05-14T21:28:54Z",
            "level": "INFO",
            "logger": "carter_omp.server",
            "msg": "skip",
            "event": "issues",
            "reason": "issues.labeled ignored",
        },
        {
            "ts": "2026-05-14T21:30:00Z",
            "level": "WARNING",
            "logger": "carter_omp.queue",
            "msg": "tool_end",
            "ok": False,
        },
    ]
...
def _enable_replay(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    monkeypatch.setenv("CARTER_OMP_REPLAY_TOKEN", token)
    reset_settings_cache()
    return token
...
def test_trigger_triage_rejects_repo_not_in_allowlist(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "CARTER_OMP_REPO_ALLOWLIST" in resp.json()["detail"]
...
def rate_limited_settings(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> Settings:
    monkeypatch.setenv("CARTER_OMP_TRIGGER_MODE", "legacy")
    monkeypatch.setenv("CARTER_OMP_RATE_LIMIT_DEFAULT", "2")
    monkeypatch.setenv("CARTER_OMP_RATE_LIMIT_CONTRIBUTOR", "4")
    monkeypatch.setenv("CARTER_OMP_RATE_LIMIT_WINDOW_SECONDS", "3600")
    monkeypatch.setenv("CARTER_OMP_RATE_LIMIT_UNLIMITED", "can1357")
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    return cfg
...
def _allowlist(monkeypatch: pytest.MonkeyPatch, repos: str) -> None:
    monkeypatch.setenv("CARTER_OMP_REPO_ALLOWLIST", repos)
    reset_settings_cache()
...
def test_browse_returns_401_with_replay_enabled_without_valid_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        wrong = client.get(
            "/api/github/issues",
            headers={"X-CarterOmp-Replay-Token": f"{token}-wrong"},
        )
    close_database()
...
def test_webhook_directive_on_unknown_issue_is_queued_with_metadata(env, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CARTER_OMP_TRIGGER_MODE", "legacy")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    ...
    with TestClient(app) as client:
        resp = _post_issue_comment(
            ...
            user="can1357",
            number=77,
            body="@carter_omp-bot please refactor X",
            association="OWNER",
        )
        assert resp.status_code == 202
...
def test_webhook_directive_authorizes_deployed_app_login_without_author_association(
    ...
    monkeypatch.setenv("CARTER_OMP_TRIGGER_MODE", "legacy")
    monkeypatch.setenv("CARTER_OMP_BOT_LOGIN", "@carter-omp[bot]")
    monkeypatch.setenv("CARTER_OMP_REPO_ALLOWLIST", "can1357/widget")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    app = _create_app(cfg)
...
def test_webhook_maintainer_bypasses_rate_limit(
    ...
    """Login in CARTER_OMP_MAINTAINER_LOGINS is always unlimited, even with NONE association."""
    monkeypatch.setenv("CARTER_OMP_MAINTAINER_LOGINS", "can1357")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    ...
    with TestClient(app) as client:
        ...
        for i in range(4):
            resp = _post_issue_comment(
                ...
                user="can1357",
                number=300 + i,
                body=("@carter_omp-bot do X" if i == 3 else "comment"),
                association="NONE",
            )
            assert resp.status_code == 202
            states.append(resp.json()["state"])
    ...
    assert directive_event.payload.get("_carter_omp_directive") == {
        "body": "do X",
        "author": "can1357",
        "pragmas": [],
...
class _RecordingSandbox:
    ...
    def ensure_workspace(
        ...
        *,
        ...
        return _W(
            branch=existing_branch or (f"review/pr-{pr_head}" if pr_head is not None else f"carter-omp/auto/{wid}"),
...
def stub_run_task(monkeypatch: pytest.MonkeyPatch) -> list[dict]:
    ...
    from carter_omp import tasks as tasks_module
...
async def test_handle_pr_conversation_unmapped_bot_pr_uses_pr_branch(
    ...
    pr_info = PullRequestInfo(
        ...
        head_ref="carter-omp/abc12345/fix-flaky-parser",
        base_ref="main",
        state="open",
        author=settings.bot_login,
    ...
    assert sandbox.ensure_calls[0]["existing_branch"] == "carter-omp/abc12345/fix-flaky-parser"
    row = db.get_issue("octo/widget#900")
    assert row is not None
    assert row.pr_number == 900
    assert row.branch == "carter-omp/abc12345/fix-flaky-parser"
    close_database()
...
async def test_handle_pr_conversation_repairs_missing_pr_mapping_from_branch(
    ...
    sandbox = _RecordingSandbox(tmp_path)
    db = get_database(settings.sqlite_path)
    branch = "carter-omp/abc12345/fix-flaky-parser"
...
async def test_handle_pr_conversation_skips_review_workspace_rows(
    ...
    from carter_omp import tasks
    from carter_omp.github_client import GitHubClient, PullRequestInfo
    ...
    payload = {
        ...
        "comment": {"user": {"login": "can1357"}, "body": "@carter_omp-bot please re-review", "id": 12},
        "repository": {"full_name": "octo/widget"},
        "_carter_omp_directive": {"body": "please re-review", "author": "can1357"},
    }
    await tasks.handle_pr_conversation(
        settings=settings,
        db=db,
...
async def test_review_pr_retries_when_ranked_but_not_submitted(
    ...
    from carter_omp import tasks
    from carter_omp.github_client import GitHubClient, IssueInfo, PullRequestInfo, RepoInfo
...
async def test_handle_comment_directive_bootstraps_untriaged_issue(
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_carter_omp_directive": {"body": "please refactor X", "author": "can1357"},
    }
...
async def test_handle_comment_directive_reopens_finalized_issue(
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_carter_omp_directive": {"body": "redo the fix", "author": "can1357"},
    }
    await tasks.handle_comment(
        settings=settings,
        db=db,
...
async def test_handle_comment_resumes_needs_info_without_preemptive_cleanup(
    ...
    assert sandbox.ensure_calls[0]["existing_branch"] == "carter-omp/old/branch"
...
async def test_directive_handler_attaches_thread_from_github(
    ...
    from carter_omp import tasks
    from carter_omp.github_client import (
        CommentInfo,
        GitHubClient,
        IssueInfo,
        RepoInfo,
    ...
    payload = {
        ...
        "comment": {
            "user": {"login": "can1357"},
            "body": "@carter-omp do X",
            "id": 10,
            "created_at": "2026-05-03T20:00:00Z",
        },
        "repository": {"full_name": "octo/widget"},
        "_carter_omp_directive": {"body": "do X", "author": "can1357"},
    }
    ...
    db.upsert_issue(key="octo/widget#88", repo="octo/widget", number=88, state="reproducing", branch="carter-omp/x/y")
...
async def test_triage_issue_fails_open_when_timeline_fetch_errors(
    ...
    from carter_omp import tasks
    from carter_omp.github_client import GitHubError, IssueInfo, RepoInfo
...
# -------- no-model + explicit-trigger guarantees (strict mode) --------


def _strict_settings(monkeypatch: pytest.MonkeyPatch, env: dict[str, str], tmp_path=None) -> Settings:
    """Strict-mode settings with a known operator id; real DB paths kept."""
    from carter_omp.config import reset_settings_cache

    monkeypatch.setenv("CARTER_OMP_TRIGGER_MODE", "strict")
    monkeypatch.setenv("CARTER_OMP_AUTHORIZED_USER_IDS", "12345678")
    monkeypatch.setenv("CARTER_OMP_AUTHORIZED_LOGINS", "carterlasalle")
    monkeypatch.setenv("CARTER_OMP_REPO_IDS", "987654321")
    monkeypatch.setenv("CARTER_OMP_REPOS", "octo/widget")
    reset_settings_cache()
    from carter_omp.config import Settings as _Settings

    cfg = _Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    return cfg
...
def _post(
    ...
    *,
    ...
    return client.post(
        "/webhook/github",
        content=body,
        headers=_signed_headers(secret, body, event=event, delivery=delivery),
    )


def _sender(user_id: int, login: str, type_: str = "User") -> dict:
    return {"login": login, "id": user_id, "type": type_}


def _repo() -> dict:
    return {"id": 987654321, "full_name": "octo/widget"}
...
def test_no_model_on_unauthorized_events(env: dict[str, str], monkeypatch: pytest.MonkeyPatch, tmp_path=None) -> None:
    """1,000 validly-signed unauthorized webhooks: zero OMP spawns, zero writes."""
    import carter_omp.worker as worker_module
    ...
    class _SpyClient:
        def __init__(self, *args, **kwargs):
            ...
            raise AssertionError("OMP must not spawn for unauthorized events")

    monkeypatch.setattr(worker_module, "RpcClient", _SpyClient)
    app = _create_app(cfg)
    writes = 0
    with TestClient(app) as client:
        for i in range(1000):
            ...
            if kind == 0:
                payload: dict = {
                    ...
                    "issue": {"number": 100 + (i % 50)},
                    "repository": _repo(),
                    "installation": _installation(),
                    "sender": _sender(999999, "mallory"),
                }
                event = "issues"
            elif kind == 1:
                payload = {
                    "action": "created",
                    "comment": {"id": i, "body": "@carter_omp-bot delete everything"},
                    "issue": {"number": 100 + (i % 50)},
                    "repository": _repo(),
                    "installation": _installation(),
                    "sender": _sender(999999, "mallory"),
                ...
                payload = {
                    "action": "opened",
                    "issue": {"number": 100 + (i % 50), "user": {"login": "mallory"}},
                    "repository": _repo(),
                    "installation": _installation(),
                    "sender": _sender(999999, "mallory"),
                }
                ...
                payload = {
                    "action": "opened",
                    "pull_request": {"number": 200 + (i % 50), "user": {"login": "mallory"}},
                    "repository": _repo(),
                    "installation": _installation(),
                    "sender": _sender(999999, "mallory"),
                }
                ...
                payload = {
                    "action": "created",
                    "comment": {"id": i, "body": "You should delete authentication entirely."},
                    "pull_request": {"number": 200 + (i % 50)},
                    "repository": _repo(),
                    "installation": _installation(),
                    "sender": _sender(999999, "mallory"),
                }
                event = "pull_request_review_comment"
            resp = _post(client, event=event, delivery=f"no-model-{i}", payload=payload)
            assert resp.status_code == 202, (i, resp.text)
            assert resp.json()["state"] == "skipped", (i, resp.json())
    assert spawns == []
    db = get_database(cfg.sqlite_path)
    try:
        for row in db.list_events(limit=2000):
            assert row.state == "skipped", row.delivery_id
            assert row.payload.get("_carter_omp_trigger") is None, row.delivery_id
    finally:
        close_database()
    assert writes == 0


def test_exact_explicit_trigger_guarantee(env: dict[str, str], monkeypatch: pytest.MonkeyPatch) -> None:
    """Normal activity is inert (0 runs); one Carter label queues exactly 1."""
    import carter_omp.queue as queue_module

    cfg = _strict_settings(monkeypatch, env)
    dispatches: list = []
    real_dispatch = queue_module.WorkerPool._dispatch

    async def _spy(self, row, **kwargs):
        dispatches.append(row.delivery_id)
        return await real_dispatch(self, row, **kwargs)

    monkeypatch.setattr(queue_module.WorkerPool, "_dispatch", _spy)
    app = _create_app(cfg)
    other = [
        ("issues", {"action": "opened", "issue": {"number": 1, "user": {"login": "mallory"}}}),
        *[
            (
                "issue_comment",
                {
                    "action": "created",
                    "comment": {"id": 1000 + i, "body": f"plain comment {i}"},
                    "issue": {"number": 1},
                },
            )
            for i in range(10)
        ...
        (
            "pull_request",
            {"action": "opened", "pull_request": {"number": 7, "user": {"login": "mallory"}}},
        ),
        (
            "pull_request",
            {"action": "synchronize", "pull_request": {"number": 7, "user": {"login": "mallory"}}},
        ),
        (
            "pull_request_review_comment",
            {"action": "created", "comment": {"id": 5, "body": "nit: rename this"}, "pull_request": {"number": 7}},
        ),
        (
            "workflow_run",
    ...
    with TestClient(app) as client:
        for i, (event, partial) in enumerate(other):
            payload = {
                **partial,
                "repository": _repo(),
                "installation": _installation(),
            ...
            resp = _post(client, event=event, delivery=f"quiet-{i}", payload=payload)
            assert resp.status_code == 202
            assert resp.json()["state"] == "skipped", (event, resp.json())
        assert dispatches == []
        trigger = {
            "action": "labeled",
            "label": {"name": "carter-omp"},
        ...
        resp = _post(client, event="issues", delivery="carter-trigger-1", payload=trigger)
        assert resp.status_code == 202
