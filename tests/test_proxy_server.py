"""HMAC + endpoint coverage for the github-proxy FastAPI app."""
...
from carter_omp.proxy_hmac import HEADER_RUN_TOKEN, HEADER_SIGNATURE, HEADER_TIMESTAMP, sign
from carter_omp.run_token import mint_run_token
from carter_omp.sandbox import workspace_key
...
def _build_settings(tmp_path: Path) -> Settings:
    ...
    cfg = Settings.model_construct(
        ...
        bot_login="carter_omp-bot",
        git_author_email="carter_omp-bot@example.invalid",
        repo_allowlist_raw="octo/widget",
        github_proxy_url=None,
        github_proxy_hmac_key=SecretStr(_HMAC),
        github_proxy_bind_host="0.0.0.0",
        github_proxy_bind_port=8081,
        workspace_root=tmp_path / "workspaces",
        sqlite_path=tmp_path / "carter-omp.sqlite",
...
def _signed(
    ...
    *,
    ...
    headers = {HEADER_TIMESTAMP: timestamp, HEADER_SIGNATURE: sig}
    if run_token is not None:
        headers[HEADER_RUN_TOKEN] = run_token
    return headers
...
def _run_token(
    *,
    ...
    token = mint_run_token(
        key=key or _HMAC.encode(),
        run_id="run-test",
        repo_id=repo_id,
        repo=repo,
        issue=issue,
        branch=branch,
        capabilities=capabilities or {"comment", "label", "push_branch", "open_pr", "request_review", "review_pr"},
        now=(_time.time() - 3600) if expired else None,
    )
    if tamper:
        body, sig = token.rsplit(".", 1)
        token = body + "." + ("0" * len(sig))
    return token
...
def test_read_remote_urls_uses_safe_directory_and_slot_identity(
    ...
    from carter_omp.proxy import server as proxy_server

    captured: dict[str, object] = {}
    repo_dir = tmp_path / "repo"
    monkeypatch.setenv("GITHUB_TOKEN", "parent-token")
    monkeypatch.setenv("CARTER_OMP_GIT_HTTP_AUTH", "parent-auth")
    ...
    monkeypatch.setattr("carter_omp.proxy.server.subprocess.run", fake_run)
    monkeypatch.setattr(
        "carter_omp.proxy.server._slot_subprocess_kwargs",
    ...
    assert "CARTER_OMP_GIT_HTTP_AUTH" not in env
...
async def test_hmac_accept_post_comment_round_trip(proxy_settings: Settings) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/post_comment", body, run_token=_run_token(issue=None)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 200
    assert resp.json() == {"id": 42, "author": "carter_omp-bot", "body": "hello", "created_at": "2026-01-01T00:00:00Z"}
...
async def test_authenticated_login(proxy_settings: Settings) -> None:
    def gh(req: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json={"login": "carter_omp-bot"})
    ...
    assert resp.status_code == 200
    assert resp.json() == {"login": "carter_omp-bot"}
...
async def test_add_issue_labels(proxy_settings: Settings) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/add_issue_labels", body, run_token=_run_token(issue=None)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 200
...
async def test_remove_issue_label(proxy_settings: Settings) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/remove_issue_label", body, run_token=_run_token(issue=None)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 200
...
async def test_add_assignees(proxy_settings: Settings) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/add_assignees", body, run_token=_run_token(issue=None)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 200
...
async def test_close_issue(proxy_settings: Settings) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/close_issue", body, run_token=_run_token(issue=None)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 200
...
async def test_open_pull_request(proxy_settings: Settings) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/open_pull_request", body, run_token=_run_token(issue=None, branch="feature")),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 200
...
async def test_request_reviewers(proxy_settings: Settings) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/request_reviewers", body, run_token=_run_token(issue=None)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 200
...
async def test_git_clone_creates_pool_dir(proxy_settings: Settings, upstream_repo: Path) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/git/clone", body, run_token=_run_token(issue=None)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 200
...
async def test_git_clone_github_url_passes_scoped_token(
    ...
    monkeypatch.setattr("carter_omp.proxy.server.git_clone", fake_git_clone)
    app = _build_app(proxy_settings)
...
async def test_git_fetch_repairs_missing_alternate_and_bad_ref(proxy_settings: Settings, upstream_repo: Path) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/git/fetch", body, run_token=_run_token(issue=None)),
                "Content-Type": "application/json",
            },
        )
...
async def test_git_fetch_github_origin_uses_explicit_scoped_remote(
    ...
    monkeypatch.setattr("carter_omp.proxy.server.git_fetch_prune", fake_fetch_prune)
    app = _build_app(proxy_settings)
    body = b'{"repo":"octo/widget"}'
    async with await _async_client(app) as client:
        resp = await client.post(
...
async def test_git_push_happy_path(proxy_settings: Settings, upstream_repo: Path) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            headers={
                **_signed(
                    "POST", "/gh/v1/git/push", body, run_token=_run_token(issue=None, branch="carter-omp/abc/feature")
                ),
                "Content-Type": "application/json",
            },
        )
...
async def test_git_push_passes_slot_uid_to_git_push(
    ...
    from carter_omp.git_ops import PushResult

    branch = "carter-omp/abc/slot"
    ...
    monkeypatch.setattr("carter_omp.proxy.server.git_push", fake_git_push)
    app = _build_app(proxy_settings)
    body = (
        b'{"repo":"octo/widget","workspace_key":"octo__widget__1","branch":"'
        + branch.encode()
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            headers={
                **_signed(
                    "POST", "/gh/v1/git/push", body, run_token=_run_token(issue=None, branch="carter-omp/abc/slot")
                ),
                "Content-Type": "application/json",
            },
        )
...
async def test_git_push_rejects_invalid_slot_uid(proxy_settings: Settings, slot_uid: int) -> None:
    app = _build_app(proxy_settings)
    body = (
        b'{"repo":"octo/widget","workspace_key":"octo__widget__1","branch":"carter-omp/abc/x","expected_head":"'
        + (b"0" * 40)
        + b'","slot_uid":'
        + str(slot_uid).encode()
        + b"}"
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/git/push", body, run_token=_run_token(issue=None, branch="carter-omp/abc/x")),
                "Content-Type": "application/json",
            },
        )
...
async def test_git_push_head_drift(proxy_settings: Settings, upstream_repo: Path) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            headers={
                **_signed(
                    "POST", "/gh/v1/git/push", body, run_token=_run_token(issue=None, branch="carter-omp/abc/drift")
                ),
                "Content-Type": "application/json",
            },
        )
...
async def test_git_push_workspace_key_mismatch(proxy_settings: Settings) -> None:
    app = _build_app(proxy_settings)
    body = (
        b'{"repo":"octo/widget","workspace_key":"other__repo__1","branch":"carter-omp/abc/x","expected_head":"'
        + (b"0" * 40)
        + b'"}'
    )
    async with await _async_client(app) as client:
        ...
        resp = await client.post(
            ...
            headers={
                **_signed(
                    ...
                    "/gh/v1/git/push_release",
                    body,
                    run_token=_run_token(issue=None, capabilities={"update_default_branch", "move_release_tag"}),
                ),
                "Content-Type": "application/json",
            },
        )
...
async def test_oversized_content_length_rejected_with_413(proxy_settings: Settings) -> None:
    ...
    """
    proxy_settings.github_proxy_max_body_bytes = 256  # type: ignore[misc]
    app = _build_app(proxy_settings, lambda _: httpx.Response(500, json={}))
    payload = b"x" * 1024
    headers = {
        **_signed("POST", "/gh/v1/post_comment", payload, run_token=_run_token(issue=None)),
        "Content-Type": "application/json",
...
async def test_streamed_body_above_cap_rejected_with_413(proxy_settings: Settings) -> None:
    ...
    proxy_settings.github_proxy_max_body_bytes = 64  # type: ignore[misc]
    app = _build_app(proxy_settings, lambda _: httpx.Response(500, json={}))
...
async def test_git_push_rejects_attacker_origin(proxy_settings: Settings, upstream_repo: Path) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/git/push", body, run_token=_run_token(issue=None)),
                "Content-Type": "application/json",
            },
        )
...
async def test_git_push_rejects_origin_with_wrong_repo(proxy_settings: Settings, upstream_repo: Path) -> None:
    ...
    branch = "carter-omp/abc/mismatch"
    repo_dir, head = _stage_workspace(proxy_settings, upstream_repo, "octo/widget", 1, branch)
...
async def test_git_fetch_ref_rejects_injection_refs(proxy_settings: Settings, bad_ref: str) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/git/fetch_ref", body, run_token=_run_token(issue=None)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 400, resp.text
...
# ============================================================================
# Run-token attack tests (S25/S75)
# ============================================================================


def _attack_token(**kwargs: object) -> str:
    defaults: dict[str, object] = {"repo": "octo/widget", "issue": 1}
    defaults.update(kwargs)
    return _run_token(**defaults)  # type: ignore[arg-type]


async def test_run_token_wrong_repo_rejected(proxy_settings: Settings) -> None:
    app = _build_app(proxy_settings, lambda _: httpx.Response(201, json={"id": 1}))
    body = b'{"repo":"octo/widget","number":1,"body":"hi"}'
    async with await _async_client(app) as client:
        resp = await client.post(
            "/gh/v1/post_comment",
            content=body,
            headers={
                **_signed("POST", "/gh/v1/post_comment", body, run_token=_attack_token(repo="evil/other")),
                "Content-Type": "application/json",
            },
        )
...
async def test_run_token_wrong_thread_rejected(proxy_settings: Settings) -> None:
    app = _build_app(proxy_settings, lambda _: httpx.Response(201, json={"id": 1}))
    body = b'{"repo":"octo/widget","number":999,"body":"hi"}'
    async with await _async_client(app) as client:
        resp = await client.post(
            "/gh/v1/post_comment",
            content=body,
            headers={
                **_signed("POST", "/gh/v1/post_comment", body, run_token=_attack_token(issue=1)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 403
...
async def test_run_token_missing_capability_rejected(proxy_settings: Settings) -> None:
    app = _build_app(proxy_settings, lambda _: httpx.Response(201, json={"id": 1}))
    body = b'{"repo":"octo/widget","number":1,"body":"hi"}'
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/post_comment", body, run_token=_attack_token(capabilities={"label"})),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 403
...
async def test_run_token_expired_rejected(proxy_settings: Settings) -> None:
    app = _build_app(proxy_settings, lambda _: httpx.Response(201, json={"id": 1}))
    body = b'{"repo":"octo/widget","number":1,"body":"hi"}'
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/post_comment", body, run_token=_attack_token(expired=True)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 401
...
async def test_run_token_tampered_rejected(proxy_settings: Settings) -> None:
    app = _build_app(proxy_settings, lambda _: httpx.Response(201, json={"id": 1}))
    body = b'{"repo":"octo/widget","number":1,"body":"hi"}'
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/post_comment", body, run_token=_attack_token(tamper=True)),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 401
...
async def test_run_token_missing_rejected(proxy_settings: Settings) -> None:
    app = _build_app(proxy_settings, lambda _: httpx.Response(201, json={"id": 1}))
    body = b'{"repo":"octo/widget","number":1,"body":"hi"}'
    async with await _async_client(app) as client:
        resp = await client.post(
...
async def test_push_outside_namespace_rejected(proxy_settings: Settings, upstream_repo: Path) -> None:
    app = _build_app(proxy_settings)
    body = (
        b'{"repo":"octo/widget","workspace_key":"octo__widget__1","branch":"main","expected_head":"' + b"0" * 40 + b'"}'
    )
    async with await _async_client(app) as client:
        resp = await client.post(
            "/gh/v1/git/push",
            content=body,
            headers={
                **_signed("POST", "/gh/v1/git/push", body, run_token=_attack_token(branch="main")),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 403
...
async def test_release_push_without_release_caps_rejected(proxy_settings: Settings) -> None:
    ...
    async with await _async_client(app) as client:
        resp = await client.post(
            ...
            content=body,
            headers={
                **_signed("POST", "/gh/v1/git/push_release", body, run_token=_attack_token()),
                "Content-Type": "application/json",
            },
        )
    assert resp.status_code == 403
...
async def test_proxy_enforces_allowlist_independently(proxy_settings: Settings) -> None:
    proxy_settings.repo_allowlist_raw = "other/repo"
    app = _build_app(proxy_settings, lambda _: httpx.Response(200, json={}))
    async with await _async_client(app) as client:
        resp = await client.get(
            "/gh/v1/repo",
...
async def test_app_token_provider_mints_scoped_tokens() -> None:
    import subprocess

    from carter_omp.app_auth import AppTokenProvider, mint_app_jwt

    key = subprocess.run(["openssl", "genrsa", "2048"], capture_output=True, text=True, check=True).stdout
    jwt = mint_app_jwt(app_id="123", private_key_pem=key)
    assert len(jwt.split(".")) == 3

    seen: list[dict] = []

    def handler(req: httpx.Request) -> httpx.Response:
        seen.append({"url": str(req.url), "body": req.content.decode()})
        return httpx.Response(200, json={"token": "tok-abc", "expires_at": "2030-01-01T00:00:00Z"})

    provider = AppTokenProvider(app_id="123", private_key_pem=key, transport=httpx.MockTransport(handler))
    first = provider.token_for_repo(installation_id=9, repo="octo/widget")
    second = provider.token_for_repo(installation_id=9, repo="octo/widget")
    assert first == "tok-abc" == second
    assert len(seen) == 1  # second call served from cache
    assert "widget" in seen[0]["body"]


async def test_proxy_prefers_app_mode_when_configured(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import subprocess
