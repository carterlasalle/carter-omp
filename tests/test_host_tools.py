...
from carter_omp.capabilities import ISSUE_RUN_CAPABILITIES as ALL_CAPS
from carter_omp.capabilities import PR_REVIEW_CAPABILITIES as REVIEW_CAPS
from carter_omp.capabilities import RELEASE_RUN_CAPABILITIES as RELEASE_CAPS
...
from carter_omp.github_client import GitHubClient, IssueIndexEntry, IssueInfo, RepoInfo
from carter_omp.host_tools import AbortController, ReleaseToolContext, ToolBindings, build
...
def test_repo_command_env_scrubs_secrets_and_uses_workspace_cache(
    ...
    monkeypatch.setenv("CARTER_OMP_GH_PROXY_HMAC_KEY", "secret-proxy")
    ...
    assert env["CARTER_OMP_GH_PROXY_HMAC_KEY"] == ""
    ...
    def fake_run_repo_command(
        ...
        if command[:3] == ["git", "log", "--format=%H%x09%ae%x09%an"]:
            return subprocess.CompletedProcess(
                command,
                0,
                "abc123\tcarter_omp-bot@example.invalid\tcarter_omp-bot\n",
                "",
            )
...
def test_gh_post_comment_happy_path(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            201, json={"id": 999, "user": {"login": "carter_omp-bot"}, "body": "hi", "created_at": "t"}
        )
...
def test_gh_post_comment_defaults_to_inbound_pr_thread(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        captured["url"] = str(request.url)
        return httpx.Response(201, json={"id": 7, "user": {"login": "carter_omp-bot"}, "body": "hi", "created_at": "t"})
...
def test_repro_record_chowns_to_slot_when_root(db: Database, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "carter_omp.host_tools.os.chown",
...
def test_mark_unable_posts_comment_marks_needs_info_and_labels_issue(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            201, json={"id": 321, "user": {"login": "carter_omp-bot"}, "body": "x", "created_at": "t"}
        )
...
def test_review_tools_reject_outside_review_mode(db: Database, tmp_path: Path) -> None:
    ...
    try:
        tool = host_tools._build_pr_review_comment(bindings)
        with pytest.raises(RpcCommandError):
...
def test_impl_gate_rejects_unauthorized_non_auto_classification_before_repo_commands(
    ...
    bindings, loop, t = _bindings(db, tmp_path, httpx.MockTransport(lambda _r: httpx.Response(500)))
    bindings = replace(bindings, capabilities=frozenset())
...
def test_impl_gate_ignores_skipped_authorized_event(
    ...
    monkeypatch.setattr(host_tools, "_run_repo_command", record_repo_command)
    try:
        tool = host_tools._build_push_branch(bindings)
        with pytest.raises(RpcCommandError) as exc:
            tool.execute({}, _ctx())
    finally:
        _stop_loop(loop, t)

    assert "lacks capability" in str(exc.value)
    assert calls == []
...
def test_classify_issue_renames_branch_when_slug_provided(db: Database, tmp_path: Path) -> None:
    ...
    assert head == "refs/heads/carter-omp/abc12345/fix-windows-env-colon-vars"
...
def test_classify_issue_rejects_invalid_branch_slug(db: Database, tmp_path: Path) -> None:
    ...
    assert bindings.workspace.branch == "carter-omp/abc12345/some-issue"
...
def test_classify_issue_rename_failure_does_not_apply_labels_or_classification(
    ...
    assert row.branch == "carter-omp/abc12345/some-issue"
    assert bindings.workspace.branch == "carter-omp/abc12345/some-issue"
...
def test_gh_push_branch_rejects_wrong_identity(db: Database, tmp_path: Path) -> None:
    ...
    try:
        ...
        assert "carter_omp-bot <carter_omp-bot@example.invalid>" in msg
        ...
        assert not any(r.startswith("refs/heads/carter-omp/") for r in refs.stdout.splitlines()), refs.stdout
    finally:
        _stop_loop(loop, thread)
...
def test_gh_push_branch_rejects_invalid_identity_scan_range(db: Database, tmp_path: Path) -> None:
    ...
    seed.mkdir()
    env = os.environ | {
        "GIT_AUTHOR_NAME": "carter_omp-bot",
        "GIT_AUTHOR_EMAIL": "carter_omp-bot@example.invalid",
        "GIT_COMMITTER_NAME": "carter_omp-bot",
        "GIT_COMMITTER_EMAIL": "carter_omp-bot@example.invalid",
    }
    subprocess.run(["git", "init", "--initial-branch=main", str(seed)], check=True, capture_output=True)
    (seed / "README.md").write_text("init\n")
    for cmd in (
        ...
        [
            ...
            str(seed),
            "-c",
            "user.email=carter_omp-bot@example.invalid",
            "-c",
            "user.name=carter_omp-bot",
            "commit",
            "-m",
            "init",
        ],
    ...
    def handler(_request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            json={
                "number": 7,
                "html_url": "https://github.com/octo/widget/pull/7",
                "head": {"ref": "carter-omp/abc12345/some-issue"},
                "base": {"ref": "main"},
            },
        )
...
def test_gh_push_branch_runs_fix_and_check_before_pushing(
    ...
    assert lines[0] == "carter_omp-bot <carter_omp-bot@example.invalid> feat: follow-up", lines
    assert lines[1] == "carter_omp-bot <carter_omp-bot@example.invalid> init", lines
...
def test_gh_push_branch_skip_capability_bypasses_failing_bun_check(
    db: Database, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """SKIP_CHECKS capability bypasses a failing `bun check` and pushes anyway.
    ...
    try:
        ...
        bindings = replace(bindings, capabilities=bindings.capabilities | {Capability.SKIP_CHECKS})
        tool = next(x for x in build(bindings) if x.name == "gh_push_branch")
        result = tool.execute({}, _ctx())
    finally:
        _stop_loop(loop, thread)
    ...
    assert any(r.startswith("refs/heads/carter-omp/") for r in refs.stdout.splitlines()), refs.stdout
...
def test_gh_push_branch_skip_capability_still_refuses_dirty_worktree(
    db: Database, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """SKIP_CHECKS capability MUST still refuse when there are uncommitted changes —
...
def test_gh_open_pr_runs_fix_then_check_and_amends_formatter_diff(
    ...
    lines = log.stdout.strip().splitlines()
    assert lines[0] == "carter_omp-bot|carter_omp-bot@example.invalid|feat: initial change"
...
def test_gh_open_pr_skip_capability_bypasses_failing_bun_run_test(
    db: Database, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """SKIP_CHECKS capability bypasses the suite alongside fix/check.
...
def test_gh_open_pr_refuses_dirty_worktree_before_fix(
    ...
    # Origin's carter-omp/* branch was never created — push refused before reaching the network.
    refs = subprocess.run(
        ["git", "-C", str(bare), "for-each-ref", "--format=%(refname)"],
        capture_output=True,
        text=True,
    ...
    assert not any(r.startswith("refs/heads/carter-omp/") for r in refs.stdout.splitlines())
...
def _stub_settings(*, enabled: bool = True, hours: float = 4.0):
    ...
    return Settings.model_construct(
        ...
        github_proxy_url="http://proxy.invalid",
        github_proxy_hmac_key=SecretStr("k" * 32),
...
def _question_handler(captured: dict[str, Any]):
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            201,
            json={"id": 4242, "user": {"login": "carter_omp-bot"}, "body": "x", "created_at": "t"},
        )
...
def _release_bindings(
    ...
    *,
    ...
    author_name: str = "carter_omp-bot",
    author_email: str = "carter_omp-bot@example.invalid",
    dirty: bool = False,
    ...
    _release_git("config", "user.name", "carter_omp-bot", cwd=repo_dir)
    _release_git("config", "user.email", "carter_omp-bot@example.invalid", cwd=repo_dir)
    if create_fix:
