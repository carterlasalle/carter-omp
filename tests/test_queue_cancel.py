...
from carter_omp.cancellation import (
    clear_current_event,
...
async def test_start_reaps_configured_slot_uids(
    ...
    calls: list[int] = []
    monkeypatch.setattr("carter_omp.queue._reap_slot", lambda uid: calls.append(uid))
    pool = WorkerPool(
        settings=settings,
        db=db,
        github=_StubGitHub(),  # type: ignore[arg-type]
...
async def test_run_event_reaps_slot_before_release(
    ...
    monkeypatch.setattr("carter_omp.queue._reap_slot", lambda uid: order.append(("reap", uid)))
