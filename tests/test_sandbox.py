...
def test_workspace_key_and_branch_shape() -> None:
    ...
    assert branch.startswith("carter-omp/")
    parts = branch.split("/")
...
def test_rename_workspace_branch_renames_local_branch(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    repo_dir = root / "repo"
    initial = "carter-omp/abc12345/some-issue"
    ...
    assert new_branch == "carter-omp/abc12345/fix-json-bom"
    assert ws.branch == "carter-omp/abc12345/fix-json-bom"
    head = subprocess.run(
    ...
    assert head == "refs/heads/carter-omp/abc12345/fix-json-bom"
...
def test_rename_workspace_branch_refreshes_shared_metadata(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "carter_omp.sandbox._share_git_metadata_with_slots",
...
def test_rename_workspace_branch_runs_git_as_slot_when_permissions_active(
    ...
    monkeypatch.setattr("carter_omp.sandbox.subprocess.run", fake_run)
    monkeypatch.setattr("carter_omp.sandbox._share_git_metadata_with_slots", lambda _repo_dir, _slot_uid: None)

    new_branch = rename_workspace_branch(ws, "fix-json-bom", slot_uid=2004)

    assert new_branch == "carter-omp/abc12345/fix-json-bom"
    assert captured["cmd"] == ["git", "branch", "-m", initial, "carter-omp/abc12345/fix-json-bom"]
...
def test_rename_workspace_branch_is_idempotent_when_slug_unchanged(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    repo_dir = root / "repo"
    initial = "carter-omp/abc12345/keep-me"
    _init_worktree_repo(repo_dir, initial)
    ws = Workspace(
        root=root,
        repo_dir=repo_dir,
...
def test_rename_workspace_branch_noop_when_pr_open(tmp_path: Path) -> None:
    ...
    root = tmp_path / "ws"
    repo_dir = root / "repo"
    initial = "carter-omp/abc12345/old-slug"
    _init_worktree_repo(repo_dir, initial)
    ws = Workspace(
        root=root,
        repo_dir=repo_dir,
...
def test_rename_workspace_branch_surfaces_git_failure(tmp_path: Path) -> None:
    root = tmp_path / "ws"
    repo_dir = root / "repo"
    initial = "carter-omp/abc12345/old"
    _init_worktree_repo(repo_dir, initial)
    # Create a second branch that collides with the rename target.
    _git(["-C", str(repo_dir), "branch", "carter-omp/abc12345/new"], cwd=repo_dir.parent)
    ws = Workspace(
        root=root,
        repo_dir=repo_dir,
        session_dir=root / ".omp-session",
...
def test_delete_bad_refs_removes_worktree_holding_the_ref(tmp_path: Path) -> None:
    ...
    from carter_omp.git_ops import _delete_bad_refs

    pool = tmp_path / "pool"
    _init_worktree_repo(pool, "main")

    # Create a worktree on `carter-omp/badhex/bad-branch`.
    work_dir = tmp_path / "worktree"
    subprocess.run(
        ["git", "worktree", "add", "-b", "carter-omp/badhex/bad-branch", str(work_dir), "main"],
        cwd=str(pool),
        check=True,
        capture_output=True,
        text=True,
    ...
    fetch_output = (
        ...
        "fatal: bad object refs/heads/carter-omp/badhex/bad-branch\n"
    ...
    rp = subprocess.run(
        ["git", "rev-parse", "--verify", "refs/heads/carter-omp/badhex/bad-branch"],
        cwd=str(pool),
        capture_output=True,
        text=True,
    )
...
def test_ensure_workspace_creates_worktree(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    assert ws.branch.startswith("carter-omp/")
...
def test_chown_workspace_noops_when_not_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("carter_omp.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("carter_omp.sandbox.os.geteuid", lambda: 1000)
    monkeypatch.setattr(
        "carter_omp.sandbox.subprocess.run",
...
def test_chown_workspace_noops_off_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("carter_omp.sandbox.platform.system", lambda: "Darwin")
    monkeypatch.setattr("carter_omp.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr(
        "carter_omp.sandbox.subprocess.run",
        lambda cmd, *, check: calls.append((cmd, check)),
...
def test_reap_slot_noops_when_permissions_inactive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("carter_omp.sandbox.platform.system", lambda: "Darwin")
    monkeypatch.setattr("carter_omp.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("carter_omp.sandbox.os.kill", lambda pid, sig: calls.append((pid, sig)))
...
def test_reap_slot_kills_slot_uid_on_linux_root(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("carter_omp.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("carter_omp.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("carter_omp.sandbox._slot_pids", lambda _uid: (111, 222))
    monkeypatch.setattr("carter_omp.sandbox.os.kill", lambda pid, sig: calls.append((pid, sig)))
...
def test_prepare_slot_tmpdir_mkdirs_without_chown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("carter_omp.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("carter_omp.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("carter_omp.sandbox.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))
...
def test_chown_workspace_normalizes_to_root_when_slots_disabled(
    ...
    monkeypatch.setattr("carter_omp.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("carter_omp.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("carter_omp.sandbox.os.getegid", lambda: 0)
    monkeypatch.setattr("carter_omp.sandbox.subprocess.run", fake_run)
...
def test_prepare_slot_runtime_env_returns_workspace_private_paths_without_chown(
    ...
    monkeypatch.setattr("carter_omp.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("carter_omp.sandbox.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))
    monkeypatch.setattr("carter_omp.sandbox.subprocess.run", lambda cmd, **_kwargs: calls.append(cmd))
...
def test_ensure_workspace_refreshes_permissions_for_retry_slot_and_session(
    ...
    monkeypatch.setattr("carter_omp.sandbox._chown_workspace", record_chown)
    monkeypatch.setattr("carter_omp.sandbox._share_git_metadata_with_slots", record_share)
...
def test_ensure_workspace_preserves_checked_out_branch_on_replay(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    renamed = "carter-omp/abc12345/renamed"
...
def test_ensure_workspace_existing_branch_starts_from_remote_head(tmp_path: Path, upstream_repo: Path) -> None:
    branch = "carter-omp/abc12345/existing-pr"
...
def test_remove_workspace_prunes_pool_after_failed_worktree_remove(
    ...
    monkeypatch.setattr("carter_omp.sandbox._safe_run", fake_safe_run)
...
def test_redact_credentials_strips_userinfo() -> None:
    from carter_omp.sandbox import redact_credentials
...
def test_git_command_error_redacts_url_in_args_and_stderr(tmp_path: Path) -> None:
    ...
    from carter_omp.sandbox import _run
...
def test_push_force_with_lease_succeeds_after_local_amend(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    from carter_omp.git_ops import push as git_push
    ...
    _git(["-C", str(work), "checkout", "-b", "carter-omp/abc/topic"], cwd=tmp_path)
    (work / "x.txt").write_text("a\n")
    _git(["-C", str(work), "add", "x.txt"], cwd=tmp_path)
    _git(["-C", str(work), "commit", "-m", "initial"], cwd=tmp_path)
    git_push(work, branch="carter-omp/abc/topic", expected_head=None, token=None)

    # Amend (rewrites the SHA at origin/carter-omp/abc/topic).
    ...
    result = git_push(work, branch="carter-omp/abc/topic", expected_head=None, token=None)
    assert result.head == amended

    # Origin's branch ref now matches the amended SHA.
    on_origin = subprocess.run(
        ["git", "-C", str(upstream_repo), "rev-parse", "refs/heads/carter-omp/abc/topic"],
        capture_output=True,
        text=True,
        check=True,
    ).stdout.strip()
...
def test_push_force_with_lease_refuses_when_origin_moved(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    force-pushing."""
    from carter_omp.git_ops import GitCommandError
    from carter_omp.git_ops import push as git_push
    ...
    _git(["-C", str(intruder), "checkout", "-b", "carter-omp/abc/topic", "origin/carter-omp/abc/topic"], cwd=tmp_path)
    ...
    _git(["-C", str(intruder), "push", "--force", "origin", "carter-omp/abc/topic"], cwd=tmp_path)
...
def test_run_git_injects_safe_directory_and_subprocess_identity(
    ...
    from carter_omp.git_ops import _run_git
    ...
    monkeypatch.setattr("carter_omp.git_ops.subprocess.run", fake_run)
...
def test_run_git_scopes_token_and_scrubs_parent_auth_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from carter_omp.git_ops import AUTH_ENV_VAR, _run_git
...
def test_ensure_workspace_populates_from_natives_cache(tmp_path: Path, upstream_repo: Path) -> None:
    from carter_omp.natives_cache import NativesCache, compute_key, target_triple
...
def test_ensure_workspace_acquires_repo_lock(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("carter_omp.sandbox._run", lambda *a, **k: ok)
    monkeypatch.setattr("carter_omp.sandbox._safe_run", lambda *a, **k: ok)
    monkeypatch.setattr("carter_omp.sandbox._chown_workspace", lambda *a, **k: None)
    monkeypatch.setattr("carter_omp.sandbox._share_git_metadata_with_slots", lambda *a, **k: None)
    monkeypatch.setattr("carter_omp.sandbox._provision_runtime_dirs", lambda *a, **k: None)
...
def test_safe_run_timeout_returns_124(monkeypatch: pytest.MonkeyPatch) -> None:
    import carter_omp.sandbox as s
    ...
    monkeypatch.setattr("carter_omp.sandbox.subprocess.run", boom)
...
def test_ensure_workspace_raises_when_local_branch_probe_times_out(
    ...
    monkeypatch.setattr("carter_omp.sandbox._safe_run", fake_safe_run)
    monkeypatch.setattr("carter_omp.sandbox._run", lambda *a, **k: subprocess.CompletedProcess(["x"], 0, "", ""))
    monkeypatch.setattr("carter_omp.sandbox._chown_workspace", lambda *a, **k: None)
    monkeypatch.setattr("carter_omp.sandbox._share_git_metadata_with_slots", lambda *a, **k: None)
    monkeypatch.setattr("carter_omp.sandbox._provision_runtime_dirs", lambda *a, **k: None)
...
def test_ensure_workspace_raises_when_symbolic_ref_probe_times_out(
    ...
    monkeypatch.setattr("carter_omp.sandbox._share_git_metadata_with_slots", lambda *a, **k: None)
    monkeypatch.setattr("carter_omp.sandbox._provision_runtime_dirs", lambda *a, **k: None)
    monkeypatch.setattr("carter_omp.sandbox._git_env_for_repo", lambda *a, **k: {})
...
def test_worktree_add_cleans_partial_state_on_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("carter_omp.sandbox._run", boom_run)
    monkeypatch.setattr("carter_omp.sandbox._safe_run", fake_safe_run)
