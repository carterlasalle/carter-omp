"""Contract test for the carter_omp server API response vs status-contract.json.

Regenerate the committed fixture with:
    CARTER_OMP_UPDATE_STATUS_CONTRACT=1 pytest tests/test_status_contract.py
"""
...
def test_cancel_errors_and_gating(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        resp = client.post(
            "/api/cancel",
            json={"delivery_id": "run-cancel-2"},
            headers={"X-CarterOmp-Replay-Token": "bad-token"},
        )
