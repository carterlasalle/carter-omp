...
LEGACY = TriggerPolicy(mode="legacy")


def route_legacy(event_type: str, payload: object, **kwargs: object) -> RouteDecision:
    return route(event_type, payload, policy=LEGACY, **kwargs)  # type: ignore[arg-type]
...
def test_route_incoming_pr_opened_skips_draft_bot_and_disabled() -> None:
    ...
    assert not route_legacy("pull_request", payload, allowlist=ALLOWLIST, bot_login=BOT).should_queue
    ...
    disabled = route_legacy("pull_request", payload, allowlist=ALLOWLIST, bot_login=BOT, pr_review_enabled=False)
    ...
    decision = route_legacy(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@carter_omp-bot please re-review",
            },
            "issue": {"number": 9, "user": {"login": "contributor"}, "pull_request": {"url": "x"}},
            "repository": {"full_name": "octo/widget"},
        },
...
def test_extract_mention_returns_body_minus_mention() -> None:
    assert extract_mention("hey @carter_omp-bot please look", "carter_omp-bot") == "hey please look"
    assert extract_mention("@carter_omp-bot do X", "carter_omp-bot") == "do X"


@pytest.mark.parametrize("configured_login", ["@carter-omp", "carter-omp[bot]", "@carter-omp[bot]"])
def test_extract_mention_accepts_prefixed_or_app_bot_login(configured_login: str) -> None:
    assert extract_mention("@carter-omp go ahead", configured_login) == "go ahead"


def test_extract_mention_strips_literal_app_suffix_from_body() -> None:
    assert extract_mention("@carter-omp[bot] go ahead", "carter-omp[bot]") == "go ahead"


def test_extract_mention_rejects_extended_literal_app_suffix() -> None:
    assert extract_mention("@carter-omp[bot]-helper go ahead", "carter-omp[bot]") is None


def test_extract_mention_returns_none_without_mention() -> None:
    assert extract_mention("hello there", "carter_omp-bot") is None
    assert extract_mention(None, "carter_omp-bot") is None
    assert extract_mention("", "carter_omp-bot") is None


def test_extract_mention_is_case_insensitive() -> None:
    assert extract_mention("yo @CARTER_OMP-BOT", "carter_omp-bot") == "yo"


def test_extract_mention_respects_hyphen_word_boundary() -> None:
    # @carter_omp-bot-helper must NOT match @carter_omp-bot.
    assert extract_mention("@carter_omp-bot-helper hi", "carter_omp-bot") is None


def test_extract_mention_handles_multiple_occurrences() -> None:
    assert extract_mention("@carter_omp-bot one, then @carter_omp-bot two", "carter_omp-bot") == "one, then two"
    ...
    decision = route_legacy(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@carter_omp-bot go ahead and push",
            },
            "issue": {"number": 9},
    ...
    decision = route_legacy(
        "issue_comment",
        {
            ...
            "comment": {
                ...
                "body": "@carter_omp-bot go ahead with the plan",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
    ...
    decision = route_legacy(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@carter_omp-bot change the indentation in foo.py",
            },
    ...
    decision = route_legacy(
        "pull_request_review_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@carter_omp-bot use a generator here",
            },
    ...
    decision = route_legacy(
        "issue_comment",
        {
            ...
            "comment": {
                "user": {"login": "can1357"},
                "author_association": "OWNER",
                "body": "@carter_omp-bot /model gpt /thinking low\nrefactor X",
            },
            "issue": {"number": 9},
            "repository": {"full_name": "octo/widget"},
        },
