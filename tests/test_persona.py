...
from carter_omp.worker import DirectiveInfo, ReleaseTaskContext, ThreadMessage
...
@dataclass(slots=True, frozen=True)
class _Workspace:
    branch: str = "carter-omp/abc/test"
...
class _Comment:
    id: int = 1
    author: str = "can1357"
    body: str = "@carter-omp please fix"
...
def test_followup_comment_prompt_embeds_thread_context() -> None:
    thread = (
        ThreadMessage(kind="pr_body", author="carter-omp", body="PR body", created_at=""),
...
def test_resume_triage_renders_branch_and_issue() -> None:
    ...
    assert "carter-omp/abc/test" in out
