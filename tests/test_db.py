...
from carter_omp.db import Database, iso_seconds_ago, issue_key
...
def test_upsert_issue_round_trip(db: Database) -> None:
    ...
    row = db.upsert_issue(
        ...
        number=7,
        state="opened",
        branch="carter-omp/abcd1234/some-issue",
        session_dir="/tmp/s",
        pr_number=42,
    )
    assert row.state == "opened"
    assert row.branch == "carter-omp/abcd1234/some-issue"
    ...
    by_branch = db.find_issue_by_branch("octo/widget", "carter-omp/abcd1234/some-issue")
...
def test_migration_adds_classification_to_existing_db(tmp_path: Path) -> None:
    ...
    conn.executescript(
        ...
        INSERT INTO issues VALUES ('octo/widget#1', 'octo/widget', 1, 'carter-omp/x', '/tmp/s', NULL,
...
def test_record_run_and_authorization_decision(tmp_path) -> None:
    ...
    try:
        ...
        db.record_run(
            ...
            policy_version="carter-omp/v1",
            capabilities_json='["comment"]',
            authorized_at="2026-01-01T00:00:00+00:00",
        )
        row = db._conn.execute("SELECT * FROM runs WHERE id='r1'").fetchone()
        assert row["actor_login"] == "carterlasalle"
        dec = db._conn.execute("SELECT * FROM authorization_decisions WHERE delivery_id='d1'").fetchone()
