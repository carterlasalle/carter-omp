...
def test_settings_load_from_env(env: dict[str, str]) -> None:
    ...
    assert cfg.bot_login == "carter_omp-bot"
...
def test_settings_missing_required(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    ...
    monkeypatch.setenv("CARTER_OMP_GH_PROXY_URL", "")
...
def test_orchestrator_mode_loads_proxy_config(env: dict[str, str]) -> None:
    ...
    assert cfg.github_proxy_url == "http://github-proxy.invalid:8081"
    assert cfg.github_proxy_hmac_key is not None
    assert cfg.github_proxy_hmac_key.get_secret_value().startswith("test-hmac-key")
...
def test_proxy_mode_loads_pat(proxy_env: dict[str, str]) -> None:
    ...
    assert cfg.github_proxy_url is None
    assert cfg.github_proxy_hmac_key is None


def test_allowlist_csv_parsing(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    monkeypatch.setenv("CARTER_OMP_REPO_ALLOWLIST", "  alpha/one ,beta/two, ,gamma/three ")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
...
def test_real_replay_token_preserved(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    monkeypatch.setenv("CARTER_OMP_REPLAY_TOKEN", "abc")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
...
def test_blank_bot_login_rejected(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    monkeypatch.setenv("CARTER_OMP_BOT_LOGIN", "   ")
    reset_settings_cache()
    with pytest.raises(ValidationError):
        Settings()  # type: ignore[call-arg]
...
def test_bot_login_normalizes_mention_case_and_app_suffix(
    ...
    monkeypatch.setenv("CARTER_OMP_BOT_LOGIN", raw_login)
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    assert cfg.bot_login == "carter-omp"


def test_maintainer_logins_normalize_csv_entries(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    monkeypatch.setenv("CARTER_OMP_MAINTAINER_LOGINS", " can1357, @CARTER-OMP , @Alice[bot] ,, ")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    assert cfg.maintainer_logins == frozenset({"can1357", "carter-omp", "alice"})


@pytest.mark.parametrize(
    ("raw_login", "expected"),
    [
        ("carter-omp", "carter-omp"),
        (" @carter-omp ", "carter-omp"),
        (" @CARTER-OMP ", "carter-omp"),
        ("carter-omp[bot]", "carter-omp"),
        ("@carter-omp[bot]", "carter-omp"),
        (" @CARTER-OMP[BOT] ", "carter-omp"),
    ],
)
def test_maintainer_logins_common_entry_forms(
    ...
    monkeypatch.setenv("CARTER_OMP_MAINTAINER_LOGINS", raw_login)
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
...
def test_pick_model_covers_full_pool(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    ...
    monkeypatch.setenv("CARTER_OMP_MODEL", "a,b,c")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
...
def test_release_model_falls_back_to_general_pool(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    monkeypatch.setenv("CARTER_OMP_MODEL", "a")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
...
def test_release_model_pool_csv_parses(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    monkeypatch.setenv("CARTER_OMP_MODEL", "fallback")
    monkeypatch.setenv("CARTER_OMP_RELEASE_MODEL", " release-a, release-b ,, ")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
...
def test_task_timeout_hard_grace_env_parses(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    monkeypatch.setenv("CARTER_OMP_TASK_TIMEOUT_HARD_GRACE_SECONDS", "12.5")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    assert cfg.task_timeout_hard_grace_seconds == 12.5


def test_strict_mode_requires_authorized_user_ids(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    from carter_omp.config import reset_settings_cache

    monkeypatch.setenv("CARTER_OMP_TRIGGER_MODE", "strict")
    monkeypatch.setenv("CARTER_OMP_AUTHORIZED_USER_IDS", "")
    reset_settings_cache()
    with pytest.raises(ValidationError, match="AUTHORIZED_USER_IDS"):
        Settings()  # type: ignore[call-arg]


def test_strict_mode_requires_repo_ids(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    from carter_omp.config import reset_settings_cache

    monkeypatch.setenv("CARTER_OMP_TRIGGER_MODE", "strict")
    monkeypatch.setenv("CARTER_OMP_REPO_IDS", "")
    reset_settings_cache()
    with pytest.raises(ValidationError, match="REPO_IDS"):
        Settings()  # type: ignore[call-arg]


def test_strict_mode_rejects_ambient_triggers(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    from carter_omp.config import reset_settings_cache

    monkeypatch.setenv("CARTER_OMP_TRIGGER_MODE", "strict")
    monkeypatch.setenv("CARTER_OMP_AUTO_ISSUE_TRIAGE", "true")
    reset_settings_cache()
    with pytest.raises(ValidationError, match="ambient triggers"):
        Settings()  # type: ignore[call-arg]


def test_strict_mode_rejects_bot_in_allowlist(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> None:
    from carter_omp.config import reset_settings_cache

    monkeypatch.setenv("CARTER_OMP_TRIGGER_MODE", "strict")
    monkeypatch.setenv("CARTER_OMP_AUTHORIZED_LOGINS", "carter_omp-bot")
    reset_settings_cache()
