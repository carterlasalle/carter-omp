...
Wire carter_omp to **review pull requests opened by contributors** (and other bots), in two
phases: (1) classify + rank, (2) a real line-by-line review posted as one GitHub review.
carter_omp **never merges, closes, approves, or pushes** — the rank label is the verdict; the
...
|---|---|---|---|---|
| `triage_issue` | `issues.opened` | fresh `carter-omp/<hex>/<slug>` worktree | `gh_open_pr` / `mark_unable_to_reproduce` / `abort_task` | **The fresh-entry template** for `review_pr`. |
...
- **`GitTransport` protocol** — add:
  ...
  `fetch_base_ref` does). `ProxyGitTransport`: add the matching github-proxy git op (mirror its
...
- github-proxy mode (`proxy_client.py` + `src/proxy/server.py`): add `/gh/v1/pr_files` (GET) and
...
Add `pr_review_enabled: bool = True` (`CARTER_OMP_PR_REVIEW_ENABLED`) so the whole flow can be
...
Do **not** enable the integration smoke (`CARTER_OMP_INTEGRATION=1`) in the default suite.
