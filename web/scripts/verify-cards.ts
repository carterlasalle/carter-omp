#!/usr/bin/env node
/**
 * CarterOmp Lifecycle Cards Verification Harness.
...
// Optional browser driver: static import would make routine Yarn commands require it.
const PUPPETEER_MODULE: string = "puppeteer-core";

async function loadPuppeteer() {
  try {
    return (await import(PUPPETEER_MODULE)).default;
  } catch (error) {
    throw new Error('This verifier requires the optional "puppeteer-core" dependency. Install it to run this script.', { cause: error });
  }
}
...
// Resolve a Chrome/Chromium binary. Walks the explicit path, PATH lookups, and
// well-known absolute locations; each candidate is confirmed with fs.stat so a
// directory or stale entry never wins.
  ...
  for (const name of ["google-chrome-stable", "google-chrome", "chromium", "chromium-browser", "chrome"]) {
    for (const directory of process.env.PATH?.split(path.delimiter) ?? []) {
      ...
      try {
        ...
        // not on PATH — try the next directory
      }
    }
  }
  candidates.push(
...
async function main(): Promise<void> {
  ...
  const outDir = path.resolve(dirname, "../verify-out/carter_omp-lifecycle-cards");
  ...
  page.on("request", async (req: InterceptedRequest) => {
  ...
  statusMock = {
    ...
    runtime: {
      ...baseStatus.runtime,
      repo_allowlist: ["octo/widget", "octo/carter_omp"],
    },
  };
  ...
  results.push({
    ...
    ok: emptyText.includes("No active work") && emptyText.includes("watching octo/widget, octo/carter_omp"),
  ...
  const applyTheme = async (themeName: "dark" | "light") => {
    await page.evaluate((theme) => {
      localStorage.setItem("omp-carter_omp-theme", theme);
  ...
  // 5. Read-only pass
  // Fetch the real index.html from server, and replace the carter_omp-config script tag
  const resp = await fetch(BASE_URL);
  let html = await resp.text();
  html = html.replace(
    /<script id="carter_omp-config" type="application\/json">[^<]*<\/script>/,
    '<script id="carter_omp-config" type="application/json">{"replayEnabled":false,"replayToken":""}</script>'
  );
  ...
  statusDelayMs = 600;
  await page.goto(BASE_URL, { waitUntil: "domcontentloaded" });
  await sleep(150); // inside the delay window: skeleton still rendered
  ...
  // request carries an auditable `X-CarterOmp-Replay-Token: trigger-secret`
  ...
  const replayHtml = (await replayResp.text()).replace(
    /<script id="carter_omp-config" type="application\/json">[^<]*<\/script>/,
    `<script id="carter_omp-config" type="application/json">{"replayEnabled":true,"replayToken":"${REPLAY_TOKEN}"}</script>`,
  );
  ...
  await sleep(100); // wait for request and state update
  ...
  results.push({
    ...
    ok: retryReqs.length === 1 && retryReqs[0].headers["x-carter_omp-replay-token"] === REPLAY_TOKEN && statusTextA.includes("queued retry: fail-del-1"),
    detail: `Matching retry requests: ${retryReqs.length}, token: "${retryReqs[0]?.headers["x-carter_omp-replay-token"] ?? ""}". Status text: "${statusTextA}"`,
  });
  ...
  results.push({
    ...
    ok: cancelReqs.length === 1 && cancelReqs[0].headers["x-carter_omp-replay-token"] === REPLAY_TOKEN && statusTextB.includes("cancel signaled: run-del- (fired=false)"),
    detail: `Matching cancel requests: ${cancelReqs.length}, token: "${cancelReqs[0]?.headers["x-carter_omp-replay-token"] ?? ""}". Status text: "${statusTextB}"`,
  });
  ...
  results.push({
    ...
    ok: triageReqs.length === 1 && triageReqs[0].headers["x-carter_omp-replay-token"] === REPLAY_TOKEN,
    detail: `Matching triage requests: ${triageReqs.length}, token: "${triageReqs[0]?.headers["x-carter_omp-replay-token"] ?? ""}"`,
  });
  ...
  results.push({
    ...
    ok: retryLatestReqs.length === 1 && retryLatestReqs[0].headers["x-carter_omp-replay-token"] === REPLAY_TOKEN,
    detail: `Matching retry-latest requests: ${retryLatestReqs.length}, token: "${retryLatestReqs[0]?.headers["x-carter_omp-replay-token"] ?? ""}"`,
  });
  ...
  results.push({
    ...
    ok: enterReqs.length === 1 && enterReqs[0].headers["x-carter_omp-replay-token"] === REPLAY_TOKEN,
    detail: `Matching enter requests: ${enterReqs.length}, token: "${enterReqs[0]?.headers["x-carter_omp-replay-token"] ?? ""}"`,
  });
  ...
  await sleep(100); // let the view flip to display:flex
  ...
  await sleep(100); // wait for the failed trigger round-trip + state update
  ...
  results.push({
    ...
    ok:
      ...
      activityRetryReqs[0].headers["x-carter_omp-replay-token"] === REPLAY_TOKEN,
    detail: `Activity status: ${JSON.stringify(activityStatus)}. Retry requests: ${activityRetryReqs.length}, token: "${activityRetryReqs[0]?.headers["x-carter_omp-replay-token"] ?? ""}"`,
  });
