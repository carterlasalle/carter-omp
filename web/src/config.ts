...
// `__CARTER_OMP_CONFIG__` sentinel in `static/index.html` with a JSON blob so the
...
function readConfig(): AppConfig {
  const node = document.getElementById("carter_omp-config");
  const text = node?.textContent?.trim();
  if (!text || text === "__CARTER_OMP_CONFIG__") {
...
export const AUTH_HEADERS: Readonly<Record<string, string>> = CONFIG.replayEnabled
  ? Object.freeze({ "X-CarterOmp-Replay-Token": CONFIG.replayToken })
  : Object.freeze({});
